 ...
 * the tests in TestJoinOperator with excessively detailed output when something
 * messes up.
 *
 *
 * Again, these tests AREN'T GRADED.
 */
public class ExtraNLJTests {
        ...
        try(Transaction transaction = d.beginTransaction()) {
            ...
            for (int l = 0; l < 4; l++) {
                for (int r = 0; r < 4; r++) {
                    ...
                    if (leftRecord.getValues().get(3).equals(rightRecord.getValues().get(3))) {
                        expectedRecords.add(joinRecords(leftRecord, rightRecord));
                    }
                }
            }
    ...
    public void test2x2PNLJFull() {
        ...
        try(Transaction transaction = d.beginTransaction()) {
            // This whole section is just to generate the tables described above
            int stringSize = 430;
            Schema schema = getStringIntIntIntSchema(stringSize);
            transaction.createTable(schema, "leftTable");
            transaction.createTable(schema, "rightTable");
            List<Record> leftRecords = new ArrayList<>();
            List<Record> rightRecords = new ArrayList<>();
            List<RecordId> leftSpacers = new ArrayList<>();
            List<RecordId> rightSpacers = new ArrayList<>();
            for (int i = 0; i < 8; i++) {
                int jValue = 0;
                int page = (i / 4) + 1;
                int rNum = i % 4;
                if (i == 4) {
                    for (int j = 0; j < 4; j++) {
                        Record leftSpacer = getStringIntIntIntRecord("left", 0, 0, 0, stringSize);
                        Record rightSpacer = getStringIntIntIntRecord("right", 0, 0, 0, stringSize);
                        leftSpacers.add(transaction.getTransactionContext().addRecord("leftTable", leftSpacer.getValues()));
            ...
            for (int i = 0; i < 4; i++) {
                transaction.getTransactionContext().deleteRecord("leftTable", leftSpacers.get(i));
                transaction.getTransactionContext().deleteRecord("rightTable", rightSpacers.get(i));
            }
    ...
    public void test2x2PNLJPartialB() {
        ...
        try(Transaction transaction = d.beginTransaction()) {
            ...
            Iterator<Record> outputIterator = joinOperator.iterator();
            checkIOs(3); // If you fail this IO check try uncommenting it and seeing what your output is.
                                 // fetchNextRecord should have to load in the 2nd right page to get before it finds
                                 // the first two matching records (see the diagram above)
    ...
    public class OutputVisualizer {
        ...
        public OutputVisualizer(int leftPages, int rightPages, List<Record> leftRecords, List<Record> rightRecords, List<Record> expectedOutput) {
            ...
            for (Record r: expectedOutput) {
                int leftPage = r.getValues().get(1).getInt();
                int leftRecord = r.getValues().get(2).getInt();
                int rightPage = r.getValues().get(5).getInt();
                int rightRecord = r.getValues().get(6).getInt();
        ...
        private List<String> createRightLabels(List<Record> rightRecords) {
            ...
            for (int i = 0; i < this.rightPages; i++) {
                int v1 = rightRecords.get(i*4).getValues().get(3).getInt();
                int v2 = rightRecords.get(i*4+1).getValues().get(3).getInt();
                int v3 = rightRecords.get(i*4+2).getValues().get(3).getInt();
                int v4 = rightRecords.get(i*4+3).getValues().get(3).getInt();;
        ...
        private boolean computeFullRun() {
            ...
            for (Record r: this.repeats) {
                int leftIndex = getIndex(r.getValues().get(1).getInt(), r.getValues().get(2).getInt());
                int rightIndex = getIndex(r.getValues().get(5).getInt(), r.getValues().get(6).getInt());
        ...
        public void add(Record expectedRecord, Record actualRecord, int num) {
            assertEquals("Your output records should have 8 values. Did you join the left and right records properly?",8, actualRecord.getValues().size());
            int actualLeftPage = actualRecord.getValues().get(1).getInt();
            int actualLeftRecord = actualRecord.getValues().get(2).getInt();
            int actualRightPage = actualRecord.getValues().get(5).getInt();
            int actualRightRecord = actualRecord.getValues().get(6).getInt();

            int expectedLeftPage = expectedRecord.getValues().get(1).getInt();
            int expectedLeftRecord = expectedRecord.getValues().get(2).getInt();
            int expectedRightPage = expectedRecord.getValues().get(5).getInt();
            int expectedRightRecord = expectedRecord.getValues().get(6).getInt();
    ...
    public Schema getStringIntIntIntSchema(int stringSize) {
        List<String> fieldNames = Arrays.asList("table", "pageNum", "recordNum", "joinValue");
        List<Type> fieldTypes = Arrays.asList(
                Type.stringType(stringSize), Type.intType(), Type.intType(), Type.intType());
        return new Schema(fieldNames, fieldTypes);
    }

    public Record getStringIntIntIntRecord(String s, int i1, int i2, int i3, int stringSize) {
        List<DataBox> values = new ArrayList<>();
        values.add(new StringDataBox(s, stringSize));
    ...
    public Record joinRecords(Record leftRecord, Record rightRecord) {
        List<DataBox> values = new ArrayList<>(leftRecord.getValues());
        values.addAll(rightRecord.getValues());
        return new Record(values);
    }
}
