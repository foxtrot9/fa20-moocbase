...
public class PrettyPrinter {
    public static void printTable(Table t) {
        ...
        printRecords(s.getFieldNames(), t.iterator(), -1);
    }

    public static void printSchema(Schema s) {
        List<Record> records = new ArrayList<>();
        List<String> names = s.getFieldNames();
        List<Type> types = s.getFieldTypes();
        for (int i = 0; i < names.size(); i++) {
            records.add(new Record(Arrays.asList(
                new StringDataBox(names.get(i).toString(), 32),
                new StringDataBox(types.get(i).toString(), 32)
            )));
        }
    ...
    public static void printRecords(List<String> columnNames, Iterator<Record> records) {
        printRecords(columnNames, records, -1);
    }

    public static void printRecords(List<String> columnNames, Iterator<Record> records, int limit) {
        ArrayList<Integer> maxWidths = new ArrayList<>();
        for(String columnName: columnNames) {
            maxWidths.add(columnName.length());
        }
        ArrayList<Record> recordList = new ArrayList<Record>();
        for(;records.hasNext() && limit != 0;limit--) {
